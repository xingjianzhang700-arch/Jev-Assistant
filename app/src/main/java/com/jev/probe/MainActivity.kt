...
class MainActivity : AppCompatActivity() {
    ...
    private fun build() {
        ...
        container.addView(text("Jev Assistant", 24f, ink, bold = true))
        container.addView(text("Reads the other person's messages beside your chat app (WhatsApp, Snapchat, Instagram, Messages) and suggests replies. You always press send yourself.",
        ...
        container.addView(permCard("Accessibility access", "Reads the message text of the open chat window", a11y) {
            startActivity(Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS))
        })
        container.addView(permCard("Display over other apps", "Shows the analysis card above the chat window", overlay) {
            startActivity(Intent(Settings.ACTION_MANAGE_OVERLAY_PERMISSION, Uri.parse("package:$packageName")))
        })
        container.addView(permCard("Autostart + no battery restrictions", "Required on Xiaomi/HyperOS, otherwise the service gets frozen and stops reading messages", null) {
            runCatching {
        ...
        container.addView(actionRow("Settings", "API key · model · relationship · transparency · chat allowlist") {
    ...
    private fun statusCard(ready: Boolean, a11y: Boolean, overlay: Boolean, key: Boolean): View {
        ...
        head.addView(text(if (ready) "Ready to go" else "Not ready yet", 16f, if (ready) green else ink, bold = true))
        c.addView(head)
        ...
        if (!prefs.contextEnabled) {
            c.addView(text("Context is off. Turn it on in Settings", 12f, sub).apply {
    ...
    private fun checkLine(label: String, ok: Boolean, okWord: String = "On", noWord: String = "Off"): View {
    ...
    private fun permCard(title: String, desc: String, granted: Boolean?, onClick: () -> Unit): View {
        ...
        row.addView(left)
        row.addView(btn(if (granted == true) "Enabled" else "Enable", granted != true, onClick))
        c.addView(row)
        return c
    }
    ...
    private fun bigToggle(on: Boolean): View {
        return TextView(this).apply {
            text = if (on) "Assistant is on · Tap to turn off" else "Assistant is off · Tap to turn on"
