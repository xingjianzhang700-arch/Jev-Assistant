 ...
 * settings ("Clear knowledge base and history"), which deletes that directory and nothing else.
 */
 ...
 * WhatsApp / Instagram / Messages, and each of those titles can be listed here.
 */
data class Contact(
    val id: String,
    val name: String,
    val aliases: List<String> = emptyList(),
    /** Package names this contact has been seen in, e.g. com.whatsapp. */
...
data class ChatContext(
     ...
     *        A contact with no relationship set simply omits the "Relationship: " line.
     */
    fun background(defaultRelationship: String): String {
        val sb = StringBuilder()
        contact?.let { c ->
            val rel = c.relationship.trim()
            if (rel.isNotEmpty()) sb.append("Relationship: ").append(rel).append('\n')
            if (c.notes.isNotBlank()) sb.append("About ").append(c.name).append(": ")
                .append(c.notes.trim()).append('\n')
            if (c.autoSummary.isNotBlank()) sb.append("Past summary: ")
