 ...
 * the user can act on ("Judge API HTTP 401: …" vs "Reply API …").
 */
object Route {
...
object HttpJson {
        ...
        while (attempt < MAX_ATTEMPTS) {
            ...
            try {
                ...
                if (code == 429 || code == 529) {
                    last = ApiException(route, code, "Service busy, retried")
                    attempt++
    ...
    private fun describe(e: Exception): String {
        ...
        return when {
            m.contains("timed out") || m.contains("timeout", true) -> "Network timed out, check your connection"
            m.contains("Unable to resolve host") -> "DNS lookup failed, wrong address or no network"
            m.contains("Failed to connect") || m.contains("ECONNREFUSED") -> "Couldn't connect to that address"
            m.contains("CertPath") || m.contains("SSL") -> "HTTPS certificate verification failed"
            else -> m
        }
    }
}
