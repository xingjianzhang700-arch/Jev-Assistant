 ...
 *
 * Feishu/Lark does not obfuscate its tree, so its adapter reads plain resource-ids.
 */
...
/** Trailing "8:11 AM" (or the Chinese AM/PM equivalent) stamp X glues onto a message. */
private val X_TAIL_TIME = Regex("""\d{1,2}[:：]\d{2}\s*(上午|下午|AM|PM|am|pm)?$""") // cjk-ok
 ...
 *
 * "你：你这个说的就是那个虚拟人物，是吗？。8:11 上午。Read。" (You: this is about that virtual character, right?) // cjk-ok
 *      → ("你", "你这个说的就是那个虚拟人物，是吗？") (sender "You", body "this is about that virtual character, right?") // cjk-ok
 * "你：他这个东西开源应该问题不大。。。Read。" (You: open-sourcing it should be fine) // cjk-ok
 *      → ("你", "他这个东西开源应该问题不大") (sender "You", body "open-sourcing it should be fine") // cjk-ok
 * "All-In：附加的帖子。。" (All-In: attached post) → ("All-In", "附加的帖子") (attached post) // cjk-ok
 *
 ...
 * (a trailing "?" the user typed) survives. Null when there is no separator or nothing is left.
 */
private fun parseXDesc(desc: String): Pair<String, String>? {
    ...
    for (tail in arrayOf("Read。", "Read", "已读。", "已读", "Seen")) { // cjk-ok
 ...
 * contentDescription ("All-In：重新写了一个😂。10:29 下午。" — All-In: rewrote it lol. 10:29 PM.). // cjk-ok
 * The date divider is a
 * TextView with no "：", so filtering on class + full width + a separator keeps
 * it out. An attachment row ("All-In：附加的帖子。。" — All-In: attached post) nests the quoted post's own // cjk-ok
 ...
 * "All-In, @all_in_2026, 你这个说的就是那…" (All-In, @all_in_2026, what you're saying is that…), so ", @" is an extra guard. // cjk-ok
 *
 * Side comes from the sender label ("你" / "You"), not geometry — every row is // cjk-ok
...
class XAdapter : ChatAppAdapter {
    ...
    override fun extract(root: AccessibilityNodeInfo, res: Resources): ChatSnapshot? {
        ...
        // DM-list-only signals: the "compose new DM" affordance, or a "Chat/
        ...
        // thread has: an actual message-row shape, or the "Message" placeholder an
