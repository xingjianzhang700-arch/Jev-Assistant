...
class ScreenCapture(
    ...
    fun capture(onResult: (Result) -> Unit) {
        ...
        if (now - lastAttemptAt < need) {
            onResult(Result.Failed(CODE_THROTTLED, "Screenshots too frequent"))
            return
        }
        lastAttemptAt = now
    ...
    private fun shoot(finish: (Result) -> Unit, done: AtomicBoolean) {
        ...
        try {
            ...
            finish(Result.Failed(CODE_INTERNAL, "Screenshot failed: ${e.javaClass.simpleName}"))
        }
    }
    ...
    private fun toBitmap(result: AccessibilityService.ScreenshotResult, window: Rect?): Result {
        ...
        return try {
            ...
            if (bmp == null) {
                Result.Failed(CODE_INTERNAL, "Screenshot failed: couldn't read the captured frame")
            } else {
            ...
            Result.Failed(CODE_INTERNAL, "Screenshot failed: ${e.javaClass.simpleName}")
        } finally {
