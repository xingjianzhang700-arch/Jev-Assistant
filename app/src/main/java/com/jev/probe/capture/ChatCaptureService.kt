...
/**
 * The live capture service (registered as SelectToSpeakService in the
 * manifest — do not rename that class). It reads whichever adapted chat app
 * is in the foreground, detects a new incoming message from the other person,
 * runs Jev analysis off the main thread, and drives the floating overlay.
 *
 ...
 * "Fill"; the user still presses send.
 */
open class ChatCaptureService : AccessibilityService() {
    ...
    private val adapters = listOf(
        ...
        InstagramAdapter(), MessagesAdapter(),
        WhatsAppAdapter(), WhatsAppBusinessAdapter(), SnapchatAdapter(),
    ).associateBy { it.pkg }
     ...
     *  a page like X's DM thread briefly shows "连接中…" (Connecting…) as `snapshot.title` // cjk-ok
    ...
    override fun onServiceConnected() {
        ...
        overlay?.onSaveContact = {
            ...
            when {
                title.isNullOrBlank() -> overlay?.toast("This chat has no title, so it can't be saved")
                isTransientTitle(title) -> overlay?.toast("The chat title hasn't loaded yet. Try again in a moment")
                else -> submit {
                    val msg = try {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        // "Read screen once (OCR)", and a bubble that is gone cannot be tapped. So we park
    ...
    private fun maybeCapture() {
        ...
        // the only way in for them is the bubble menu's "Read screen once (OCR)".
        ...
        // a transient "连接中…" (Connecting…) title for a moment right after opening a thread. // cjk-ok
        ...
        // → screenshot + OCR, subject to ScreenCapture's own >=1s throttle and
        // failure backoff.
    ...
    /** A placeholder title an app shows only for a moment (e.g. X's "连接中…" (Connecting…) // cjk-ok
    ...
    private fun runAnalysis() {
        ...
        if (analyzing) return
        if (!prefs.hasKey()) { main.post { overlay?.showError("No Judge API key set. Add one in Settings") }; return }
        analyzing = true
    ...
    /**
     * Bubble menu → "Read screen once (OCR)". Works on ANY app, adapted or not: one whole
    ...
    private fun groupOcrLines(lines: List<OcrLine>): List<Msg> {
        val usable = lines
            .filter { it.text.isNotBlank() && !isPureTime(it.text) }
    ...
    private fun finishOcrSnapshot(snapshot: ChatSnapshot, pkg: String, manual: Boolean) {
        ...
        if (snapshot.messages.isEmpty()) {
            if (manual) overlay?.showError("No text recognized on this screen")
            return
        }
    ...
    private fun fillInput(text: String) {
        submit {
            ...
            main.post {
                if (ok) overlay?.toast("Filled in. Review it, then send it yourself")
                else { copyToClipboard(text); overlay?.toast("Copied. Long-press the input box to paste") }
            }
        }
    }
    ...
    companion object {
        ...
        private const val OCR_NOTE = "OCR can't tell sides apart; every message is treated as theirs"
        ...
        private val TRANSIENT_TITLE_WORDS = listOf(
            "连接中", "正在连接", "未连接", "Connecting", // cjk-ok
            "加载中", "Loading", "同步中", "Syncing" // cjk-ok
        )
    }
}
